...
import { getPositionPreferences, registerUser } from '../api/auth';
...
const Signup = () => {
  ...
  useEffect(() => {
    const fetchPreferences = async () => {
      try {
        const data = await getPositionPreferences();
  ...
  const validateField = useCallback((name, value) => {
    ...
    switch (name) {
      ...
      case 'prn_no':
        if (!sanitizedValue) return 'PRN number is required';
        if (!validationRules.prn.test(sanitizedValue)) {
      ...
      case 'semester':
        if (!sanitizedValue) return 'Semester is required';
      ...
      case 'password':
        ...
        if (!rules.hasUppercase.test(value)) return 'Password must contain at least one uppercase letter';
        if (!rules.hasLowercase.test(value)) return 'Password must contain at least one lowercase letter';
        if (!rules.hasNumber.test(value)) return 'Password must contain at least one number';
        if (!rules.hasSpecial.test(value)) return 'Password must contain at least one special character';
        break;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const sanitizedData = Object.keys(formData).reduce((acc, key) => {
        acc[key] = key === 'password' || key === 'confirm_password' 
          ? formData[key] 
      ...
      const response = await registerUser({
        ...
        college_name: sanitizedData.college_name,
        branch: sanitizedData.branch,
        aadhar_card_no: sanitizedData.aadhar_card_no.replace(/\s+/g, ''), // Remove spaces from Aadhar
        applied_position_preference: parseInt(sanitizedData.applied_position_preference),
        prn_no: sanitizedData.prn_no,
        phone_no: sanitizedData.phone_no.replace(/\s+/g, ''), // Remove spaces from phone
        email_id: sanitizedData.email_id.toLowerCase(), // Normalize email
        semester: sanitizedData.semester,
        password: sanitizedData.password
      });

      if (response.success && response.message.status === "success") {
        ...
        setTouchedFields({});
        setErrors({});
      } else if (response.success && response.message.status === "error") {
        setErrors({ 
          submit: response.message.message || 'Registration failed. Please try again.'
        });
      } else {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      required
                      className={`w-full px-3 py-2 border rounded-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors duration-200 ${
                        errors.applied_position_preference ? 'border-red-500 bg-red-50' : 'border-gray-300 hover:border-gray-400'
                      }`}
                    >
